...
class Alergenos{
    ...
    public function getAll(){
        $result = $this->db->query("SELECT id, nombre FROM alergeno");
    ...
    public function getById($id){
        ...
        $result = $this->db->query("SELECT id, nombre FROM alergeno WHERE id = ?", [$idSaneado[0]]);
    ...
    public function create($nombre){
        ...
        $this->db->query("INSERT INTO alergeno (nombre) VALUES(?)", [$nombreSaneado]);
    ...
    public function update($id, $nombre){
        ...
        $this->db->query("UPDATE alergeno SET nombre = ? WHERE id = ?", [$nombreSaneado, $idSaneado]);
        return $this->db->query("SELECT ROW_COUNT() as affected")->fetch_assoc()['affected'];
    }

    public function delete($id){
        $idSaneado = Validator::sanear([$id]);
        $this->db->query("DELETE FROM alergeno WHERE id = ?", [$idSaneado[0]]);
        return $this->db->query("SELECT ROW_COUNT() as affected")->fetch_assoc()['affected'];
    }
}